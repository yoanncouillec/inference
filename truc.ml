...
let rec deriv (ctx:context) (term:term) (ty1:ty) (constr_list:constr list) : result =
  ...
  | App(t1, t2) ->
     ...
     let ty3 = TyVar(gensym()) in
     (match (deriv ctx t1 ty3 constr_list, deriv ctx t2 sigma constr_list) with
      | ((typed_term_list1, ctx1, constr1),
         (typed_term_list2, ctx2, constr2)) ->
         (term, ty1)::typed_term_list1@typed_term_list2,(ctx1@ctx2@ctx),((ty3,TyArrow(sigma,ty1))::constr_list@constr1@constr2))
  | Int(_) ->
     let typed_term_list = [(term, ty1)] in
     let constr_list = (ty1,TyInt)::constr_list in
     typed_term_list, ctx, constr_list
  | Var(s) ->
  ...
  | Lambda(s,body) ->
     ...
     (match (deriv ((s,sigma)::ctx) body tau constr_list) with
      | (typed_term_list1, ctx1, constr1) ->
         ((term,ty1)::typed_term_list1, ctx1, (ty1,ty2)::constr1))

let deriv_init t = deriv [] t (TyVar(gensym())) []

let _ =
  let t = App(App(Lambda("x",Lambda("y",Var("y"))),Int(12)),Int(13)) in
